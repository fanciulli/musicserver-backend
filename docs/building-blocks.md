...
| **MusicSourcePlugin (abstract)** | `src/types/plugins/music_sources.ts`                 | Extends `Plugin` with the five music-specific operations: `scan()`, `browse()`, `search()`, `stream()`, and `getAlbumArt()`. `stream()` must always resolve the song's total size alongside the `Readable` — it is used as the `content-length` for HTTP streaming and as the reported file size for SMB clients. |
